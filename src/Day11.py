...
def calculateFlash(file, rounds):
    ...
    input = np.insert(input, 0, 0, axis=0)
    input = np.insert(input, rows+1, 0, axis=0)
    input = np.insert(input, 0, 0, axis=1)
    input = np.insert(input, columns+1, 0, axis=1)
    #print(input)
    flashtimes = 0
                    ...
                    for a in range(i-1, i+2):
                        if a != 0 and a != rows+1:
                            for b in range(j-1, j+2):
                                if b != 0 and b != columns+1:
                                    if [a,b] not in flashlist:
                                        stack.append([a,b])
        while stack:
            i, j = stack.pop()
